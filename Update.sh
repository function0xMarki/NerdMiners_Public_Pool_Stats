...
if [ "$REQUIREMENTS_CHANGED" = true ]; then
    ...
    if "$SCRIPT_DIR/venv/bin/pip" install -r "$SCRIPT_DIR/requirements.txt" --quiet 2>> "$LOG_FILE"; then
        ...
        send_telegram "⚠️ <b>Dependency Update Failed</b>%0A%0AThe update changed requirements.txt, but pip failed to install the new packages.%0AThe bot will continue running with the previous dependency versions.%0A%0A⚡ <b>To fix manually, connect to your server and run:</b>%0A<code>${SCRIPT_DIR}/venv/bin/pip install -r ${SCRIPT_DIR}/requirements.txt</code>%0A%0ALogs: <code>${LOG_FILE}</code>"
    fi
fi
...
if [ -n "$ADDED_VARS" ]; then
    send_telegram "⚙️ <b>New Configuration Options</b>%0A%0AThe latest update added new settings to config.py.%0AThey are active with their default values — no action needed unless you want to customize them.%0A%0A${ADDED_VARS}%0A%0ATo change them, edit config.py on your server."
